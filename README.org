...
*** Giving /cl-tpg/ to Quicklisp so you can load it as a module.
...
#+BEGIN_SRC sh
  ln -s /fully/expanded/path/to/your/repos/cl-tpg /fully/expanded/path/to/your/quicklisp/local-projects/
#+END_SRC

*** Setting up Python and Gymnasium environments.

This repository automatically bridges to Python to interact with Gymnasium through py4cl2.
In order to use a different Python binary than your system-level Python:

#+BEGIN_SRC lisp
  (setf (py4cl2:config-var 'py4cl2:pycmd) "/path/to/your/python/binary")
  (py4cl2:save-config)
  (py4cl2:pystop)
  (py4cl2:pystart)
#+END_SRC

You have to do the above only once in your REPL to point the Python binary from the system python
to whatever python binary you want to use. This is a persistent change.
...
| File             | Purpose                                                                                                                |
| emacs/tpg.el     | Everything related to the Emacs package is here including the transient menu for starting searches, stopping searches. |
| action.lisp      | Defines what an action is: either an atomic action or a reference to another team.                                     |
| cl-tpg.asd       | ASDF package definition i.e. telling Quicklisp what dependencies are needed to load CL-TPG.                            |
| dataset.lisp     | Everything related to datasets including how to load and use them efficiently.                                         |
| globals.lisp     | Key global variables all in one place. Dynamically set by the Emacs client through the search menu                     |
| helpers.lisp     | Useful utility functions                                                                                               |
| instruction.lisp | The basic unit of any program stored as an efficient representation.                                                   |
| learner.lisp     | The combination of (program, action) pairs.                                                                            |
| main.lisp        | Everything to do with the actual evolution/generational loop.                                                          |
| migration.lisp   | When teams are received over TCP, they are added to and processed from thread-safe queues defined here.                |
| mutation.lisp    | All the mutation of teams and learners are defined here.                                                               |
| networking.lisp  | All socket-based communication from Emacs to CL-TPG or from CL-TPG node to CL-TPG node is implemented here.  |
| package.lisp     | Defines what functions are public/private and the order in which to load files during (ql:quickload)                   |
| program.lisp     | The very fast execution engine for running programs on data                                                            |
| team.lisp        | A team is a candidate solution or part of a candidate solution to the task.                                            |

* Key settings you need to change

1.)

In order for Emacs and the CL-TPG servers to communicate, you first need to set the IP address of the Emacs client.
...
Once you've done this, great! Now your /CL-TPG/ is listening to your Emacs.
...
* Enabling the *tpg* package in Emacs.

#+BEGIN_SRC sh
      C-x C-f ./emacs/tpg.el
      M-x eval-buffer
      M-x tpg
#+END_SRC

The above code opens the /tpg.el/ file from the working directory. /eval-buffer/ executes each expression in the file sequentially. /tpg/ opens two buffers (windows) within emacs.
The first buffer is *tpg* which is the main control panel for interacting wih the TPG nodes/servers. *tpg* shows things like fitness and generation for the connected nodes. The second buffer is *tpg-log* which receives messages, warnings, and errors from the nodes.

[[./figures/*tpg*.png]]

* Starting searches in TPG

When you're in the *tpg* buffer, you can press
...
The first thing you should do is enable auto-refreshing of the *tpg* panel when new fitness scores or system information arrives.
...
| Evaluation Mode            | -M        | CL-TPG supports both offline learning (learning from a dataset with no task interaction) and online learning (interacting with a simulator.) This parameter sets which mode to use. |
| Gymnasium Environment Name | -G        | This tells CL-TPG which Gymnasium environment to load, e.g. "HalfCheetah-v5"                                                                                                  |
| Dataset Name               | -F        | The name of the dataset to load. CL-TPG looks for datasets in ~/.datasets/.                                                                                                   |
