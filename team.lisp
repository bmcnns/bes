...
(defun follow-reference (tpg reference observation &key (visited nil) (learner-table (build-learner-table tpg)) (team-table (build-team-table tpg)))
  (cond ((member reference visited) (error "Cycle detected during team execution."))
        ((team-p reference) (execute-team tpg (team-id reference) observation :visited visited :learner-table learner-table :team-table team-table))
        ((learner-p reference) (let* ((goto-team-id (get-reference (learner-action reference)))
                                 ...
                                 (follow-reference tpg goto-team observation :visited visited :learner-table learner-table :team-table team-table)))
  ...
  (let* ((team (gethash team-id team-table))
    ...
    (let ((highest-bidder (elt learners (argmax bids))))
      (if (atomic-p highest-bidder)
          ...
          (follow-reference tpg highest-bidder observation :visited visited :learner-table learner-table :team-table team-table)))))
