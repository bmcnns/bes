...
(defdataset *Minimal-Pendulum-Expert-v1*
  :path "~/.datasets/Minimal-Pendulum-Expert-v1")

(defdataset *Pendulum-Expert-v1*
  :path "~/.datasets/Pendulum-Expert-v1")

(setf *dataset* (subseq *Pendulum-Expert-v1* 0 1000))

(defexperiment *Pendulum-v1*
  :batch-size 1000
  :instruction-set (ADD SUB MUL DIV SIN COS LOG EXP)
  :registers (R from 1 to 8) 
  :observations (OBS from 1 to 3)
  :output-registers (R from 1 to 1)
  ...
  :actions (actions *minimal-pendulum-expert-v1*)
...
(defparameter *experiment* *Pendulum-v1*)
(defparameter *eval-fn* (make-execute-on-dataset-fn *dataset*))
(defparameter *results-folder* (format nil "/Users/brycemacinnis/experiments/pendulum/~A/" (substitute #\- #\: (timestamp))))
