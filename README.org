...
| File             | Purpose                                                                                                                |
| emacs/bes.el     | Everything related to the Emacs package is here including the transient menu for starting searches, stopping searches. |
| action.lisp      | Defines what an action is: either an atomic action or a reference to another team.                                     |
| bes.asd          | ASDF package definition i.e. telling Quicklisp what dependencies are needed to load BES.                               |
...
| networking.lisp  | All socket-based communication from Emacs to BES or from BES node to BES node is implemented here.                     |
